...
app.use((req, res, next) => {
    res.append('Access-Control-Allow-Origin' , 'https://peaceful-tundra-13574.herokuapp.com');
...
MongoClient.connect(process.env.MONGODB_URI || "mongodb://localhost:27017/chatapp", (err, Database) => {
    if(err) {
        console.log(err);
        return false;
    }
...
app.get('/chatroom/:room', (req, res, next) => {
    ...
    chatRooms.find({name: room}).toArray((err, rooms) => {
        if(err){
            console.log(err);
            return false;
        }
